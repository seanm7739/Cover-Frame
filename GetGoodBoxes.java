...
public class GetGoodBoxes {
	...
	GetGoodBoxes (String pdbpath, 
				  double prl, double prh, 
				  double gx, double gy, double gz, 
				  int boxnum, 
				  double lr, 
				  double beta, double alpha, int rmodule) throws IOException, InterruptedException{
		...
		getSentinel(boxnum, prl, prh, gx, gy, gz);
	...
	public void getSentinel(int boxnum, double prl, double prh , double gx, double gy, double gz){
		for(int i=0 ; i<boxnum ; i++){
			Atom ansAtom = getGoodSentinel((int)(prl*(atomArray.length)), getBlueAtom(prh), gx, gy, gz);
			...
			if(ansAtom==null){
				...
				System.out.println(df.format(ansAtom.x));
				System.out.println(df.format(ansAtom.y));
	...
	public int getBlueAtom(double prh){
	...
	public Atom pushOutAns (int ans, int hindex){
		...
		double[] uv= {0, 0, 0};
		//..sentinel.blueAtom......gamma%..blueAtom
		double[][] blueArray = new double[(int) Math.ceil(atomArray.length-hindex*gamma)][2];
		...
		//get gamma%'s blueAtom to ans Unit vector
		for(int i=0 ; i<blueArray.length ; i++){
	...
	//[0]=padb, [1]=prl, [2]=prh, [3,4,5]=gx,y,z, [6]=box number, [7]=ligenr, [8]=beta, [9]=alpha
	public static void main(String[] args) throws IOException, NumberFormatException, InterruptedException{
		new GetGoodBoxes(args[0], 
