...
if($pwrl == 1){
	...
	foreach $low($lows...$lowb){
		foreach $high($highs...$highb){
			$prl = $low/100;
			$prh = $high/100;
			@goodcenter = connectboxes($r,$prl,$prh,$gsx,$gsy,$gsz,$Boxnumber,$lr,$al,$factor,$sw,$sw2,$dpr,$sw3);
			...
			foreach $i(0...$#goodcenter){
				...
				system("sh prepare_gpf_dpf.sh $l $r $gridpts[0],$gridpts[1],$gridpts[2] $spacing $goodcenter[$i][0],$goodcenter[$i][1],$goodcenter[$i][2] $low $spacing"."_$high"."_$i"."_$factor"."_$factor2"."_$dr"."_$gnes $dir $gnes");
if($pwrl == 1){
